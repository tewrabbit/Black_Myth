...
AHealthPotion::AHealthPotion()
{
    ...
    SphereComp->OnComponentBeginOverlap.AddDynamic(this, &AHealthPotion::OnSphereBeginOverlap);
    ...
    if (MatFinder.Succeeded())
    {
        ...
        DynMat->SetVectorParameterValue("Color", FLinearColor::Red);   // 红
...
void AHealthPotion::OnSphereBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
    ...
    if (OtherActor && OtherActor != this)
    {
        if (GEngine)
            GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red,
                FString::Printf(TEXT("Blood potion picked by %s"), *OtherActor->GetName()));
        Destroy();
        ...
        if (PC)
        {
            ...
            if (Wukong)
            {
                ...
                if (HUD)
                {
                    ...
                    {
                        Inv->AddItem(EItemType::HealthPotion, TEXT("Health Potion"), 1);
                    }
                }
            }
        }
